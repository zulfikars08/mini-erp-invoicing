...
export type RevenueTrendPoint = { label: string; revenue: number };
export type InvoiceStatusBreakdown = { status: InvoiceStatus; count: number };
export type TopCustomer = { customerId: string; customerName: string; paidInvoices: number; revenue: number };
export type DashboardSummary = { totalCustomers: number; totalInvoices: number; totalPaidInvoices: number; totalUnpaidInvoices: number; unpaidInvoices?: number; totalRevenue: string | number; recentInvoices: Invoice[]; revenueTrend?: RevenueTrendPoint[]; invoiceStatusBreakdown?: InvoiceStatusBreakdown[]; topCustomers?: TopCustomer[] };
